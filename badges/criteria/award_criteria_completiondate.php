 ...
 *
 */
class award_criteria_completiondate extends award_criteria {

  /* @var int Criteria [BADGE_CRITERIA_TYPE_COURSESET] */
  public $criteriatype = BADGE_CRITERIA_TYPE_COMPLETIONDATE;
  public $needs_courses = true;
  ...
  public function get_options(&$mform) {
    ...
    foreach ($this->params as $p) {
      if ($course = $DB->get_record('course', array('id' => $p['course']))) {
        ...
        if (isset($p['completiondate'])) {
          $param['completiondate'] = $p['completiondate'];
        }
        ...
        $parameter=array();
        $parameter[] =& $mform->createElement('static', 'mcompletiondate_' . $param['id'], null, get_string('issue_within_completion_time', 'badges'));
        $parameter[] =& $mform->createElement('duration', 'completiondate_' . $param['id'], null, get_string('after', 'badges'));
        $mform->setType('completiondate_' . $param['id'], PARAM_INT);
        ...
        $mform->setDefault('completiondate_' . $param['id'], $param['completiondate']);
    ...
    if (!$none) {
      ...
      $agg = array();
      $agg[] =& $mform->createElement('radio', 'agg', '', get_string('allmethodcoursescompleted', 'badges'), 1);
      $agg[] =& $mform->createElement('radio', 'agg', '', get_string('anymethodcoursescompleted', 'badges'), 2);
  ...
  public function review($userid) {
    ...
    foreach ($this->params as $param) {
      ...
      if (isset($param['completiondate'])) {
        ...
        $check_date = ($date + $param['completiondate'] > strtotime('now'));
      }

      $overall = false;
      if ($this->method == BADGE_CRITERIA_AGGREGATION_ALL) {
        if ($info->is_course_complete($userid) && $check_date) {
          $overall = true;
          continue;
        } else {
          return false;
